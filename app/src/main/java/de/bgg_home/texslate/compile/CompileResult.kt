 ...
 * ([de.bgg_home.texslate.RustBridge.tectonicCompileToFile]) zurückgibt.
 *
    ...
    companion object {
        ...
        fun nativeUnavailable(cause: Throwable): CompileResult {
            ...
            return CompileResult(
                ...
                engineError = if (de) {
                    "Native Compile-Funktion nicht verfügbar – libtexslate_native.so muss neu " +
                        "gebaut werden (./build-native.sh). Details: ${cause.message}"
                } else {
                    "Native compile function unavailable – libtexslate_native.so must be rebuilt " +
